...
项目已形成可验证的v0.1生产交付候选：核心收件、API、控制台、兼容Adapter、Compose部署、灾备与Release供应链均有真实门禁。目标服务器的DNS、ACME、SMTP STARTTLS与宿主机硬故障仍必须按部署环境单独验收；任何架构判断仍可被更强证据推翻。
...
- 固定Postfix 3.11.5真实SMTP/LMTP Integration，覆盖持久队列、进程重启、4xx重投、确认丢失重复投递和未知Recipient在SMTP RCPT阶段永久拒绝；
...
- `web/`正式Vue 3控制台，覆盖真实API Client、中文默认/英文切换、Light/Dark/System/Mist、一次性Capability、HttpOnly Session、稳定分页、邮件与附件生命周期、Sandbox HTML、错误恢复、键盘与辅助技术访问。
- Canonical Compose真实浏览器E2E，覆盖HTTPS Edge、HttpOnly Session、Postfix→LMTP→Parser收件、Sandbox HTML、附件与CSRF删除闭环。
- Canonical Compose灾备演练，覆盖Non-root Maintenance、独立Bundle Verify、原数据卷删除、空目标恢复、原Browser Session/附件读取、恢复后再次投递与零残留。
- Capability到AES-256-GCM HttpOnly Browser Session的安全交换，覆盖短生命周期、`__Host-` Cookie与状态修改请求CSRF验证。
- 有界Retention Cleanup，使用短事务、`SKIP LOCKED`、Generation Queue和文件生命周期Fence清理过期Inbox及失去最后引用的Raw MIME；物理删除失败会保留任务并在后续轮次重试。
- 单机Linux Host-native Reference Profile，固定Nginx、Certbot、Postfix、PostgreSQL与systemd运行边界，并提供Commit Epoch Layer Timestamp重写、四份确定性镜像Archive与隔离无缓存双构建验证的Linux amd64 Release Bundle。
- Docker Compose主推荐Profile，固定生产镜像Digest、独立Owner/Runtime PostgreSQL Role、幂等`db-provision → migrate → app`启动链、Secret、Healthcheck、最小内部Network、资源上限、有界日志与证书共享。
- Canonical Raw Source与附件下载API，按Ownership校验后有界流式读取；控制台支持Bearer/HttpOnly Session下载，并在打开邮件后以`message:update`标记已读。
- PostgreSQL Advisory Lease强制一个Go App进程；单进程内部保留LMTP、Parser、HTTP和下载的有界并发，不虚假声明本地文件Profile支持多Replica。
...
备份恢复、Postfix持久队列、纯Go有界MIME Parser、Content级Parser Worker、Inbox Capability、Canonical API、DuckMail Adapter、正式Vue控制台与Canonical Compose自动灾备均已通过对应固定门禁。目标服务器仍必须单独完成ACME、SMTP STARTTLS、云盘/宿主机故障与断电演练。尚未声明支持的第三方API必须按Contract逐项验证，不能用本地或CI构建通过替代生产验收。
...
门禁还会构建固定Alpine Digest与Postfix `3.11.5-r0`的Integration Image，在真实SMTP/LMTP链路验证持久Queue和重投，并在固定Linux/amd64 Go 1.26.5 Bookworm镜像中重复执行Test与Race。随后启动隔离Canonical Compose全栈完成生产浏览器E2E，再运行原数据卷删除与空目标恢复演练；结束时删除本次Container、Network、内部Volume、External Backup Volume和临时Secret。
...
每个数据库事务最多处理`MAILWISP_CLEANUP_BATCH_SIZE`个Inbox，并处理一页持久Content Deletion Task。Compose默认由App每五分钟执行；Host-native辅助Profile使用systemd Timer。手动`cleanup`会取得独占维护租约，服务运行时Fail Closed。
...
go run ./cmd/mailwisp backup verify ./backups/mailwisp-20260715
```

恢复只允许目标PostgreSQL数据库为空，且`MAILWISP_CONTENT_ROOT`不存在或是空的直接挂载目录：
...
- `PATCH /api/v1/inboxes/me/messages/{id}`，请求体`{"seen":true}`
- `GET /api/v1/inboxes/me/messages/{id}/source`
...
Session不包含Capability明文，最长不超过原Capability到期时间。它是AEAD保护的Stateless Cookie：退出会清除当前浏览器Cookie，但已复制的旧Cookie在到期或Inbox删除前仍可能有效；“永久删除邮箱”才是全部访问权的权威失效点。未配置Session Key时这些路由保持关闭，CLI与自动化仍可直接使用Canonical Bearer API。
...
所有错误使用包含稳定`code`、可读`message`与`request_id`的JSON Envelope，未分类服务端错误只在结构化日志保留根因。Capability只能访问自身Inbox；删除最后一条数据库引用时会原子写入带Generation的物理删除任务，失败可重试，Content Reconciliation继续负责异常Orphan兜底。
...
主推荐安装路径见[Docker Compose Deployment](deploy/compose/README.md)；需要systemd深度集成时使用[Host-native辅助Profile](deploy/reference/README.md)。Canonical Linux amd64 Release由GitHub Actions从干净Checkout执行完整门禁、双构建一致性、SPDX SBOM、Final Image/IaC扫描与解压后Production E2E。本地复现必须先满足`deploy/compose/versions.lock`中的精确Go、Node、npm、Compose、Buildx与BuildKit版本，再执行：

```powershell
$commit = git rev-parse HEAD
./scripts/build-release.ps1 -Version 0.1.0 -Commit $commit
```

Release Bundle携带App、Maintenance、Edge与Postfix预构建镜像；Release Compose Overlay删除全部源码Build定义并禁止拉取同名远程Tag，解压部署不依赖Go/Node源码。当前Public仓库直接使用GitHub Artifact Attestations；Artifact结构、Checksum以及Public/Private仓库的Fail-closed边界见[Release Bundle说明](docs/release-bundle.md)和[ADR 0023](docs/decisions/0023-release-supply-chain.md)。
...
`MAILWISP_HEAVY_READ_CONCURRENCY=4`限制Canonical Raw Source与附件并发流；饱和时返回`503 service_busy`和`Retry-After: 1`，避免多个大邮件读取拖垮个人服务器。该值应按目标磁盘与出口带宽Benchmark调整，不应无证据扩大。
