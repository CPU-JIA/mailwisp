...
Release Bundle携带App、Maintenance、Edge与Postfix预构建镜像；Release Compose Overlay删除全部源码Build定义并禁止拉取同名远程Tag，解压部署不依赖Go/Node源码。Artifact结构、Checksum、Attestation与私有仓库Fail-closed边界见[Release Bundle说明](docs/release-bundle.md)和[ADR 0023](docs/decisions/0023-release-supply-chain.md)。
