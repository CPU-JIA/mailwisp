...
备份恢复、Postfix持久队列、纯Go有界MIME Parser、Content级Parser Worker、Inbox Capability、Canonical API、DuckMail Adapter与正式Vue控制台均已通过对应固定门禁。Canonical Compose自动灾备已实现，仍需完成GitHub Ubuntu证据；目标服务器还必须单独完成ACME、SMTP STARTTLS、云盘/宿主机故障与断电演练。尚未声明支持的第三方API必须按Contract逐项验证，不能用本地构建通过替代生产验收。
