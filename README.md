...
备份恢复、Postfix持久队列、纯Go有界MIME Parser、Content级Parser Worker、Inbox Capability、Canonical匿名Inbox API与正式Vue控制台均已通过对应GitHub Actions固定门禁。DuckMail Adapter正在独立PR完成最终门禁；生产Postfix/TLS、完整反滥用、断电故障注入和旧数据迁移仍需继续验证，不能把当前阶段当作可发布产品。
