...
备份恢复、Postfix持久队列、纯Go有界MIME Parser与Content级Parser Worker均已在GitHub Actions固定Linux门禁通过，覆盖重启、4xx重投、确认丢失重复投递、v1→v2带数据Migration、Lease Fencing、Integration、Race、Fuzz与安全扫描；Inbox Capability仍等待当前分支的完整远端门禁，不得提前宣称通过。备份恢复仍需Reference Linux人工演练。项目尚未开放匿名Inbox创建API，也未完成生产Postfix/TLS/反滥用配置、断电故障注入、旧数据迁移、业务API和正式Vue控制台，不能把当前阶段当作可发布产品。
...
本分支已用真实Integration覆盖Postfix与Go LMTP的可靠队列边界；`deploy/postfix-test`是隔离测试资源，不是生产配置。公网域名、TLS、反滥用、DNS和生产资源限制完成前，不应直接宣称生产SMTP部署完成。
