...
备份恢复的真实Integration与Race Test已经在GitHub Actions固定Linux门禁通过，但仍需Reference Linux人工恢复演练。Postfix故障测试已经进入当前开发分支，必须等待本分支GitHub Actions全绿后才能作为已验证证据。项目仍未完成生产Postfix/TLS/反滥用配置、断电故障注入、旧数据迁移、邮件解析、业务API和正式Vue控制台，不能把当前阶段当作可发布产品。
