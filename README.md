...
备份恢复、Postfix持久队列与纯Go有界MIME Parser均已在GitHub Actions固定Linux门禁通过，覆盖重启、4xx重投、确认丢失重复投递、Integration、Race、Fuzz与安全扫描；备份恢复仍需Reference Linux人工演练。Parser Worker与结果持久化已进入当前开发分支，必须等待本分支完整远端门禁通过后才能作为已验证证据。项目仍未完成生产Postfix/TLS/反滥用配置、断电故障注入、旧数据迁移、业务API和正式Vue控制台，不能把当前阶段当作可发布产品。
