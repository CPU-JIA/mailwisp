...
备份恢复以及Postfix持久队列、重启、4xx重投、确认丢失重复投递的真实Integration与Race Test，均已在GitHub Actions固定Linux门禁通过；备份恢复仍需Reference Linux人工演练。项目仍未完成生产Postfix/TLS/反滥用配置、断电故障注入、旧数据迁移、邮件解析、业务API和正式Vue控制台，不能把当前阶段当作可发布产品。
