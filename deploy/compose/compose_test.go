...
func TestComposeProductionContract(t *testing.T) {
	tests := []struct {
		...
		{"../../.github/workflows/verify.yml", []string{"./scripts/install-compose-linux.sh", "if: always()", "production-e2e-${{ github.run_id }}-${{ github.run_attempt }}", "artifacts/production-e2e/result.json", "web/playwright-production-report", "web/test-results-production"}, nil},
	}
