...
func assertLockedAPKInputs(t *testing.T, file, subject string, want []string) {
	...
	if !strings.Contains(content, "rm -rf /tmp/mailwisp-apks /var/log/apk.log") {
		t.Fatalf("%s must remove the wall-clock APK build log after installation", subject)
	}
}
