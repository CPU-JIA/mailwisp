...
### 2.6 旧TempMail：功能已经很多，边界已经开始失控

本地旧项目已经具备Go API、Go LMTP、Postfix、PostgreSQL、PgBouncer、Redis和Nginx前端，功能覆盖域名、邮箱、邮件、验证码、链接、Token、管理员、健康检查和清理任务。它不是一个应被轻易丢弃的Demo，而是一份真实需求与迁移约束的证据库。

问题集中在结构演进。`store/postgres.go`、`ingress/server.go`和`store/tokens.go`已经成为超大文件，DNS查询进入PostgreSQL Store，`main.go`同时负责HTTP、LMTP、Job、Key文件和Lifecycle。前端单一`app.js`超过4000行、`style.css`超过8000行，大量字符串HTML、Inline Event和多代Dark Theme覆盖相互叠加。

这些问题并不说明Go、PostgreSQL或原生前端天然错误。真正原因是边界没有随着功能增长被重新组织。旧项目最有价值的部分是生产行为、数据库Invariant、LMTP样本、API错误语义和真实迁移数据；最不值得复用的是已经混合职责的文件结构。MailWisp对旧代码的态度应当像对待一个运行多年的协议实现：保留证据，逐项提取，任何复制都必须经过测试。

## 三、横向分析：当前方案分别活成了什么样

### 3.1 五种路线的核心对比

| 维度 | 旧TempMail | Cloudflare Temp Email | DuckMail | YYDS | MailWisp目标 |
|---|---|---|---|---|---|
| 主要用户 | 单机自托管 | Cloudflare用户 | 公共API客户端 | 自动化与平台用户 | 高质量自托管与API集成 |
| 公网收件 | Postfix→LMTP | Email Routing→Worker | 官方资料未完整公开 | 官方资料未完整公开 | Postfix→LMTP |
| 应用形态 | Go服务但职责混合 | Workers多产品编排 | Web/API产品 | 平台型API | Go模块化单体 |
| 持久数据 | PostgreSQL | D1 | 服务端实现未完整公开 | 服务端实现未完整公开 | PostgreSQL |
| Raw MIME | DB与文件并存 | D1 TEXT/BLOB | Source API | Source API | 内容寻址文件/S3 Adapter |
| 临时状态 | Redis | KV/Rate Limiting | 未公开 | 平台内部能力 | 进程内优先，Redis可选 |
| 原生认证 | Token与管理Key | 多套JWT/Header | Password→JWT | JWT/API Key/Temp Token | Capability Token + Session |
| 实时 | 轮询/接口 | 多种集成 | 1–2秒轮询 | Long Poll/WebSocket/Webhook | Long Poll基线，SSE优化 |
| 部署复杂度 | 多容器与双Nginx | 多Cloudflare产品配置 | 托管服务 | 托管服务 | 单机Reference Profile |
| 主要风险 | 边界腐化 | 平台锁定与历史包袱 | 实时与公开配额不明 | 产品面过宽 | 新项目需证明每项选择 |
...
四套同功能Technical Spike给出了可比较数据。原生TypeScript总Gzip约3.7 KB，Svelte约18.1 KB，Vue约27.6 KB，React约62.3 KB。原生方案在网络上明显最小，却需要最多手写DOM、Escape、事件重绑定和生命周期代码，已经出现整页`innerHTML`重渲染。旧项目的大文件历史说明这种成本会随着管理功能放大。
...
MailWisp选择Postfix，不是保守，而是从SMTP长期演进和旧项目可用链路中确认：公网队列与重试已经有成熟Owner。选择LMTP，是为了让应用可以对每个Recipient返回明确状态，并在Durable Persistence前拒绝成功。
...
选择Raw/Parsed分离，来自Cloudflare大邮件与重新解析需求，也来自旧项目同时保存Raw与Parsed字段的生产经验。选择本地内容寻址文件作为Reference Storage，来自关系数据库大BLOB的WAL与备份成本，同时尊重个人服务器不应被强制部署MinIO。
...
- PostgreSQL 18.4对旧Dump迁移、索引和查询计划的兼容。
...
这个风险与旧项目巨型文件相反，却来自同一个根源：边界增长快于验证能力。MailWisp必须允许明确舍弃功能，不能把“支持更多”误认为“架构更优”。
...
| Public SMTP | Postfix 3.11.5 | 成熟队列与旧项目经验支持，LMTP E2E待补 |
| Database | PostgreSQL 18.4 + pgx | 版本来源确认，旧Dump与Repository Spike待补 |
...
- `../tempmail`只读源代码、Migration、Compose与API实现盘点。
...
本报告使用横纵分析法：纵向追踪临时邮箱从传统MTA分工、匿名地址、密码账户、Capability Token到平台型API的演进；横向比较旧TempMail、Cloudflare Temp Email、DuckMail与YYDS的当前结构，再将两条轴交汇为MailWisp的架构判断。所有“最优”结论均受当前约束和证据范围限制，可由后续Technical Spike、Benchmark、Contract Test与恢复演练推翻。
