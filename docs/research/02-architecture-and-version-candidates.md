...
状态：2026-07-14历史候选快照（当前版本与架构以版本锁、Lockfile及已接受ADR为准）
调研日期：2026-07-14

本文件保留研究阶段的候选与当时证据，不是当前安装清单或支持承诺。最终选择已经由`go.mod`、`web/package-lock.json`、`deploy/compose/versions.lock`与`docs/decisions/`锁定；版本升级必须重新走审查和门禁。
...
下列条目只记录研究起点。它们已在后续ADR、Compatibility Contract、容量Benchmark、Production E2E与灾备演练中形成当前结论；不得把本节当作现行待办清单。
