...
- Postfix同时加入独立非内部`smtp_ingress`与内部`lmtp`网络：前者使Docker真实发布`25/tcp`，后者只连接App；PostgreSQL只在内部`database`网络，Edge只通过`frontend`连接App。
- 单一收件域直接使用`relay_transport = lmtp:inet:app:2525`，不依赖发行版可能未编译的Berkeley DB `hash:`映射；本地投递关闭，因此不加载Alias数据库。
- 测试启动PostgreSQL、一次性Runtime Role Provision、Migration、App、Edge和Postfix正式镜像，不使用Mock Repository、Vite Dev Server或后门投递接口。
- 浏览器必须通过HTTPS Edge创建Inbox，把一次性Capability交换为`Secure`、`HttpOnly`、`SameSite=Lax`的`__Host-mailwisp_session`，并在Reload后恢复。
- 邮件必须从Host Loopback SMTP进入Postfix，再经LMTP、Content Store、PostgreSQL和Parser后由Vue读取；测试覆盖Plain Text、Sandbox HTML、已读更新、附件与Raw Source下载、消息删除和Inbox删除。
