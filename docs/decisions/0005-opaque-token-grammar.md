...
状态：提议中，Capability底座已验证，完整认证体系尚未完成
日期：2026-07-15
...
PAT、Browser Session和Webhook Signing Secret Repository尚未实现；本阶段不得因为Grammar可解析就宣称这些Credential已经可用。Capability底座已经由GitHub Actions完成固定PostgreSQL 18.4 Integration、Race、Fuzz、Gitleaks四类运行时探针与GitGuardian验证，最终合并后的`main`由[Run 29382274354](https://github.com/CPU-JIA/mailwisp/actions/runs/29382274354)验证全绿。

日志、HTTP Error、Telemetry、Audit Event、PAT、Browser Session、Webhook Key Material ADR与第三方Adapter Contract Test仍未完成，因此本ADR继续保持“提议中”。当前证据只接受Token Grammar与Inbox Capability底座，不代表完整认证系统已经完成。
