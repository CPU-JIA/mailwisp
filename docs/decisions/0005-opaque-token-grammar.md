...
状态：已接受（Opaque Grammar与Inbox Capability；PAT、Account与Webhook签名不属于v0.1）
日期：2026-07-15
...
| Type | 用途 | 主要边界 |
| --- | --- | --- |
| `pat` | Personal Access Token的保留Grammar | 正式Account/PAT实现前不签发 |
| `cap` | Inbox Capability Token | 只授权单个或明确集合的临时邮箱资源 |
| `ses` | 显式Session Credential的保留Grammar | v0.1浏览器Session使用ADR 0012的AEAD HttpOnly Cookie，不签发`wisp_ses` |
...
### Opaque Bearer Token存储与验证

实际签发的PAT或Capability只在签发响应中展示一次。v0.1只签发Inbox Capability；浏览器Session是独立的短期AEAD Cookie，不使用本节的Digest Repository。Opaque Bearer Repository保存：
...
- PAT若在后续版本实现，默认90天到期、最大365天，不签发永久PAT；v0.1不开放PAT。
- Capability到期时间不得晚于对应Inbox或资源集合的最早失效时间。
- v0.1 Browser Session由Capability交换得到，使用`__Host-`前缀、`Secure`、`HttpOnly`、`SameSite=Lax`、受Inbox到期约束的短期AEAD Cookie，并对状态修改请求验证CSRF；详细协议见ADR 0012。
...
Opaque Grammar与Inbox Capability接受所需证据：
...
上述Grammar、Capability、Canonical授权与Adapter Contract已经实现并进入固定门禁。PAT/Account尚未实现，也不是v0.1临时邮箱交付范围；`whsec`仍只保留Grammar与Secret Scanner识别，正式Webhook前必须先完成独立Key Material ADR。这些未开放Credential不能被文档或Adapter伪装成可用能力。
...
- Canonical API的统一Error Envelope、Request ID、Scope与Ownership边界；
- ADR 0012定义的Browser Session/CSRF交换、恢复与显式退出；
- DuckMail、YYDS和Cloudflare Temp Email Adapter Contract Test；
- Unit、Race、Fuzz、固定PostgreSQL Integration、Gitleaks运行时Probe与GitGuardian门禁。

本ADR接受的是Opaque Grammar、Inbox Capability及其授权边界，不代表Account、PAT或Webhook功能已经存在。浏览器Session的生产实现以ADR 0012为准，而不是本文件保留的`ses` Grammar。
