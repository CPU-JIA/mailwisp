...
状态：已接受（软件持久性协议；目标基础设施硬断电仍需上线验收）
日期：2026-07-14
...
5. Parser Worker异步生成有界Header、Text、未信任HTML Source与Attachment Metadata；HTML只允许由控制台净化后放入无权限Sandbox iframe。
6. Retention与显式删除在数据库事务中移除引用，并原子写入带Generation的Content Deletion Task；Worker在文件生命周期Fence内重新核对数据库引用后删除，失败保留任务重试；Reconciliation负责异常Orphan兜底。
...
进程强杀、提交前后故障、Postfix重投、Backup/Restore、Reconciliation与LMTP E2E已证明软件协议满足接受条件。目标服务器文件系统的真实断电、云盘/宿主机硬故障以及目标磁盘的容量与尾延迟属于上线验收，不以CI进程强杀冒充，也不阻止本ADR的软件协议进入“已接受”。
...
- `serve`持有独立PostgreSQL Session的Singleton Advisory Lease；第二个App进程、`reconcile`、Backup、Restore或一次性Cleanup无法并发进入，避免本地文件生命周期Fence被跨进程绕过。
- 投递从Content Put开始持有进程内读Lease直到数据库提交；物理删除持有写Lease并重新查询引用。删除任务使用递增Generation确认，旧Worker不能吞掉内容再次复用后产生的新任务。
- 显式Message/Inbox删除与过期清理都在删除`mail_contents`的同一Transaction中写入Queue；文件删除故障测试证明任务保留且下一轮成功，内容重新被引用时文件保留并安全确认旧任务。
...
- 固定Postfix 3.11.5真实验证LMTP不可达排队、Queue Volume跨重启、4xx重投、确认丢失重复Message和未知Recipient在SMTP RCPT阶段永久拒绝。
...
- 目标文件系统上的容量、目录数量与尾延迟Benchmark；仓库内Compose容量Benchmark只提供可复现基线。
- 在更新Parser依赖、Raw上限或目标资源档位时复测恶意邮件Corpus峰值RSS；现有Parser/Worker的Raw、Header、Part、Depth、Decoded Bytes与并发边界继续由门禁强制。
