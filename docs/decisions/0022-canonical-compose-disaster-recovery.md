...
- 新增按需构建的Non-root `maintenance` Target与Compose Profile。PostgreSQL 18.4 Client及新增依赖使用精确APK URL与逐文件SHA-256锁定，在固定Alpine Digest上以无Repository/Network解析的方式安装；它不携带PostgreSQL Server或`gosu`，只持有PostgreSQL Secret，并连接Backend、挂载Content与可写Backup Storage。
...
1. 启动正式PostgreSQL、Migration、App、Nginx Edge与Postfix。
...
9. 启动完整栈，以原Browser Session验证原附件，再投递并读取新邮件。
