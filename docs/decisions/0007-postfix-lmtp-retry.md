...
6. Postfix在SMTP RCPT阶段通过LMTP地址验证探测未知Recipient；Go LMTP返回`550 5.1.1`后，Postfix立即拒绝RCPT且不创建Queue Entry，避免接受后向伪造Sender产生Backscatter。LMTP暂时不可用时RCPT返回4xx。
...
- 未知Recipient在SMTP RCPT阶段返回550且Postfix Queue保持为空；LMTP验证暂时不可用时RCPT返回450，正负缓存过期后会跟随Inbox创建与删除。
