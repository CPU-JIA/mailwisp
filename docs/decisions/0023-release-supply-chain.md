...
- Canonical Release只在固定Ubuntu 24.04 Runner从干净Checkout构建。Docker Buildx 0.35.0由官方Binary SHA-256校验安装；每次构建创建独立的BuildKit 0.31.2 Digest Builder并禁用Cache。使用Git Commit Epoch、规范化Tar Metadata与`gzip -n`，同一Job执行两次从零构建并以`cmp`证明Archive逐字节一致。
- Maintenance与Postfix分别把PostgreSQL Client所需八个APK、Postfix所需九个APK固定到精确版本URL与逐文件SHA-256，再由`apk --no-network --repositories-file /dev/null`只安装已下载集合；两者都以固定Alpine Digest为Base，Release构建不依赖浮动APKINDEX，也不继承PostgreSQL Server或`gosu`等无关攻击面。
