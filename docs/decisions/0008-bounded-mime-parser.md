...
- Sanitized HTML Policy与Sandboxed iframe，后续已由ADR 0010实现；
- Attachment独立Content Object未采用；Owned Raw Source与从Raw MIME有界解码的Attachment下载API已由ADR 0010实现；
- Parsed Message查询API与重新解析管理命令；
- 真实邮件Corpus兼容矩阵和Linux峰值RSS测量。

仍未实现的能力必须在后续Change中复用本Parser边界，不得重新引入第二套无界MIME实现。
