...
- 状态：Capability、Canonical公共API与Browser Session/CSRF边界已实现；分布式反滥用尚未纳入
- 日期：2026-07-15
...
- 匿名Inbox创建API使用可信Proxy CIDR解析客户端IP，并通过有界Token Bucket限制单IP创建速率；Reference单Replica有效，多Replica部署前必须切换共享限流或在Edge集中限流。
...
Capability底座证据已由最终合并后的`main` [Run 29383329374](https://github.com/CPU-JIA/mailwisp/actions/runs/29383329374)验证全绿。Canonical API新增的Rate Limit、资源Ownership与Transport错误映射必须在本分支GitHub Actions通过后才能视为主干证据。
