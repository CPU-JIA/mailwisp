...
`npm run test:e2e`使用Route Mock专测前端状态机；仓库根目录的`./scripts/e2e-compose.ps1`使用正式Nginx、Go、Postfix与PostgreSQL运行生产链路。`./scripts/drill-compose-recovery.ps1`进一步用两阶段Playwright验证Browser Session与附件跨空目标恢复，并证明恢复后可继续收件。三类门禁不能互相替代。
